...
const formSchema = z.object({
  ...
  value: z.string().min(1, "A medida é obrigatório"),
});
...
export const SizeForm: React.FC<SizeFormProps> = ({ initialData }) => {
  ...
  const description = initialData
    ? "Editar uma tamanho"
    : "Adicionar uma tamanho";
  const toatsMessage = initialData ? "Tamanho atualizado." : "Tamanho criada.";
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error(
        "Certifique-se de remover todos os produtos deste tamanho primeiro"
      );
    } finally {
      setLoading(false);
      setOpen(false);
